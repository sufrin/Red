...
%
%
\title{AppleRed (Version 1.02)}
\author{Bernard Sufrin}
\date{November 24th 2022\\(Revised March 26th 2025)}
...
\texttt{Red} (also known as \texttt{AppleRed}) is a no-frills
...
\texttt{scala}, and configured using \textsf{redscript}\footnote{An extensible
dialect of the Lisp family.}}. It has no pretensions to being an all-encompassing
...
insertions on the same line (between other actions) are ``bunched'',
so that their effect can be undone all at once. Consecutive cuts
or other deletions \textit{of adjacent text} are also bunched.
...
\emph{Undo} too far.}  A \textit{Save} writes the document in its
current state to the filestore with its current name; and will
(usually) preserve one or more backup copies of the file. The undo
stack is preserved over a save; but when a save is (notionally)
undone nothing happens to the filestore.
...
\begin{itemize}
\item
  \emph{FindDown} (\texttt{F7}, or \texttt{Numpad-0}) selects the
  nearest \emph{following} instance of \textbf{(F)}
\item
  \emph{FindUp} \texttt{(Shift-F7)}, or \texttt{Shift-Numpad-0}) selects the
  nearest \emph{preceeding} instance of \textbf{(F)}
\item
  \emph{FindSelDown} (\texttt{Alt-f7}, or \texttt{Alt-Numpad-0}) makes
  the current selection the \emph{Find} pattern, turns off regular
  expression interpretation, and then acts as \emph{FindDown}.
\item
  \emph{FindSelUp} (\texttt{Shift-Alt-F7}, or \texttt{Shift-Alt-Numpad-0})
...
\def\FIND{\textbf{FIND}}
\def\ALT{\textbf{Alt}}
The \texttt{C-f}  key behaves as much as possible like its behaviour in
more conventional applications. Its behaviour is summarized by the following table, in
which the selection is denoted \textbf{(S)}, and \FIND\ means selecting
the nearest \textbf{(F)} in the direction indicated by the shift key (upwards if it is
pressed).\footnote{The effect of \texttt{C-f} while editing
\FND\ is also \FIND.} 
\begin{center}
\begin{tabular}{|c|c|c|l|}
\hline
\SEL&\FND&&Effect\\\hline
nonempty&empty&&\FIND \\
nonempty&nonempty&with \ALT\ shift& \FND := \SEL; \FIND\\
nonempty&nonempty&otherwise&  \FIND\\
empty&nonempty&&\FND := \SEL; \FIND\\
empty&empty&&move editing cursor to \FND\\
\hline
\end{tabular}
\end{center}
...
that is, either:
\begin{alltt}
    Numpad-0 Numpad-.      \textrm{or}   F7 F8      \textrm{or}   C-f C-r 
\end{alltt}
